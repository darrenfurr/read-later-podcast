...
/**
 * Fetch content from URL (article or YouTube)
 * Automatically detects YouTube URLs and extracts transcript
 */
/**
 * Fetch via Jina AI Reader (handles paywalls, bot-blocked sites like Medium)
 */
async function fetchViaJina(url) {
  ...
  // Jina returns markdown — extract title from header metadata or first # heading
  const titleMatch = text.match(/^Title:\s*(.+)$/m) || text.match(/^#\s+(.+)$/m);
  const title = titleMatch ? titleMatch[1].trim() : 'Untitled Article';
  // Strip Jina header metadata (everything before first blank line after header block)
  const content = text.replace(/^(Title:|URL Source:|Published Time:|Markdown Content:)[^\n]*\n/gm, '').trim();
  return { url, title, content, wordCount: countWords(content), fetchedAt: new Date().toISOString() };
}

/**
 * Detect domains known to block scrapers (Medium, Substack, etc.)
 */
function isScraperBlocked(url) {
  return /medium\.com|substack\.com|wsj\.com|nytimes\.com|ft\.com|bloomberg\.com|netflixtechblog\.com|engineering\.fb\.com|engineering\.atspotify\.com|x\.com|twitter\.com/.test(url);
}
...
export async function fetchArticle(url) {
  ...
  // Use Jina directly for known scraper-blocked domains
  if (isScraperBlocked(url)) {
    console.log(`Fetching article via Jina (scraper-blocked domain): ${url}`);
  ...
  try {
    ...
    console.log(`  Direct fetch failed, trying with curl...`);
    
    // Fallback: use curl with better headers
    try {
      const html = execSync(
      ...
      console.log(`  curl failed, trying Jina as last resort...`);
      try {
        return await fetchViaJina(url);
      } catch (jinaError) {
        console.error(`Failed to fetch ${url}:`, error.message, curlError.message, jinaError.message);
        throw new Error('fetch failed (direct, curl, and Jina all failed)');
      }
    }
  }
}
