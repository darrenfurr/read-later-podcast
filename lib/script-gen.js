...
const MODEL = process.env.PODCAST_MODEL || 'anthropic/claude-haiku-4-5';
...
- HOST (Andrew): Warm, curious, asks great follow-up questions, genuinely interested
- EXPERT (Ava): Knowledgeable, shares insights and anecdotes, explains complex topics simply
...
function cleanText(text) {
  return text
    .replace(/\[(?!PAUSE)[^\]]+\]/g, '')  // Remove stage directions
    .replace(/\s+/g, ' ')
    .replace(/&/g, ' and ')
    .replace(/\$/g, ' dollars ')
    .replace(/%/g, ' percent ')
