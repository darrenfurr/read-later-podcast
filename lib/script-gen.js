...
function cleanText(text) {
  return text
    ...
    .replace(/\*[^*]+\*/g, '')
    // Remove standalone stage directions at start of line or after punctuation
    // Pattern: "laughs softly," or "chuckles." or "sighs expectantly"
    .replace(/(?:^|[.!?,]\s*)(?:laughs?|chuckles?|sighs?|giggles?|snorts?|grins?|smiles?|beams?|winks?|nods?|shrugs?|pauses?|clears?\s+(?:his\s+|her\s+)?throat)(?:\s+(?:softly|loudly|nervously|expectantly|deeply|quietly|slightly|knowingly|warmly|broadly|briefly|dramatically))?[.,]?\s*/gi, ' ')
