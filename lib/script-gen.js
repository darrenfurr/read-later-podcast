...
STRUCTURE:
1. INTRO: Hook the listener, introduce themselves ONCE by name (Andrew & Aria), tease what they'll learn (30 seconds)
...
function parseScript(rawScript) {
  ...
  for (const line of lines) {
    ...
    if (hostMatch) {
      ...
      // Strip out any name prefixes (Ava:, Aria:, etc.)
      let text = expertMatch[1].replace(/^(Ava|Aria|Expert):\s*/i, '').trim();
      currentText = [text];
